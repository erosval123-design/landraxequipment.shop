...
(function () {
  ...
  function write(items) {
    var lean = items.map(function (i) { return { sku: i.sku, qty: i.qty, name: i.name, price: i.price }; });
    ...
    if (endpoint) {
      ...
      document.getElementById('payBtn').addEventListener('click', function (ev) {
        ...
        fetch(endpoint, {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            items: items.map(function (i) { return { sku: i.sku, qty: i.qty, name: i.name, price: i.price }; })
          })
        }).then(function (r) {
  ...
  (function () {
    ...
    if (addr) {
      ...
      if (supplied.length) {
        addr.textContent = supplied.concat('United States').join(', ');
        addr.hidden = false;
